...
function lockWelcome(){
  ...
  const hide=()=>{const el=document.getElementById("vmWelcomeSplash");if(el){el.style.setProperty("display","none","important");el.style.setProperty("visibility","hidden","important");el.style.setProperty("opacity","0","important");el.style.setProperty("pointer-events","none","important")}};
  hide();
  if(welcomeObserver)welcomeObserver.disconnect();
  welcomeObserver=new MutationObserver(hide);
  welcomeObserver.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:["style","class"]});
}
function unlockWelcome(){
  if(welcomeObserver){welcomeObserver.disconnect();welcomeObserver=null}
  document.documentElement.classList.remove("vm-notification-blocking");
  document.body.classList.remove("vm-notification-blocking");
  const el=document.getElementById("vmWelcomeSplash");
  if(el){el.style.removeProperty("display");el.style.removeProperty("visibility");el.style.removeProperty("opacity");el.style.removeProperty("pointer-events");el.classList.remove("vmWelcomeHide")}
}
...
function createPrompt(){
  ...
  overlay.innerHTML=`<section id="vm-notification-prompt" role="dialog" aria-modal="true" aria-label="Notifications VM RADIO"><div class="vm-notification-layout"><div class="vm-logo-placeholder"><img src="./vmradio-app-logo.jpg" alt="VM RADIO" onerror="this.style.display='none'"></div><div><h2>Restez<br><span>informé</span> !</h2><div class="vm-line"></div><p class="vm-text">Activez les notifications pour ne rien manquer de nos <span>nouveautés</span>, <span>informations</span> et <span>mises à jour</span> sur <span>VM Radio</span>.</p></div></div><div class="vm-notification-actions"><button class="vm-choice" id="vm-notification-later" type="button">${clockIcon}<span>Plus tard</span></button><button class="vm-choice" id="vm-notification-accept" type="button">${bellIcon}<span>Activer</span></button></div></section>`;
...
async function activateNotifications(btn){
  try{
    ...
    if(permission!=="granted"){btn.disabled=false;btn.querySelector("span").textContent="Activer";return}
    const [{initializeApp},{getMessaging,getToken,onMessage}]=await Promise.all([import("https://www.gstatic.com/firebasejs/12.17.1/firebase-app.js"),import("https://www.gstatic.com/firebasejs/12.17.1/firebase-messaging.js")]);
    const app=initializeApp(firebaseConfig,"vmRadioNotifications");const messaging=getMessaging(app);
    const registration=await navigator.serviceWorker.register("./sw.js?vm=9",{scope:"./"});
    onMessage(messaging,payload=>{const n=payload.notification||{};if(Notification.permission==="granted"&&document.visibilityState==="visible")new Notification(n.title||"VM RADIO",{body:n.body||"Nouvelle information disponible.",icon:n.icon||"./vmradio-app-icon-192.png"})});
    const token=await getToken(messaging,{vapidKey:VAPID_KEY,serviceWorkerRegistration:registration});
    if(!token)throw new Error("Token FCM indisponible");
    localStorage.setItem("vmRadioFcmToken",token);localStorage.setItem(NOTICE_KEY,"1");finishPrompt();
  }catch(error){console.error("VM RADIO notifications:",error);btn.disabled=false;btn.querySelector("span").textContent="Activer"}
}
function setupPrompt(){
  document.querySelectorAll("#vm-notifications-card").forEach(el=>el.remove());
  if(localStorage.getItem(NOTICE_KEY)==="1")return;
  if(window.Notification&&Notification.permission==="granted"){localStorage.setItem(NOTICE_KEY,"1");return}
  lockWelcome();
  const overlay=createPrompt();
  requestAnimationFrame(()=>requestAnimationFrame(()=>{overlay.classList.add("vm-show");const accept=document.getElementById("vm-notification-accept"),later=document.getElementById("vm-notification-later");if(accept)accept.onclick=()=>activateNotifications(accept);if(later)later.onclick=chooseLater}));
}
