...
function hideWelcome(){
  ...
  if(el){el.dataset.vmNotificationHidden="1";el.style.setProperty("display","none","important");}
}
function showWelcome(){
  if(welcomeObserver){welcomeObserver.disconnect();welcomeObserver=null;}
  const el=document.getElementById("vmWelcomeSplash");
  if(el){el.style.removeProperty("display");el.style.removeProperty("visibility");el.classList.remove("vmWelcomeHide");delete el.dataset.vmNotificationHidden;}
}
function lockWelcome(){
  ...
  welcomeObserver=new MutationObserver(()=>{
    const el=document.getElementById("vmWelcomeSplash");
    if(el && !el.dataset.vmNotificationHidden){
      el.dataset.vmNotificationHidden="1";
      el.style.setProperty("display","none","important");
    }
  });
  welcomeObserver.observe(document.body,{childList:true,subtree:true,attributes:true,attributeFilter:["style","class"]});
}
...
function createPrompt(){
  let overlay=document.getElementById("vm-notification-overlay");
...
function setupPrompt(){
  ...
  if("Notification" in window&&Notification.permission==="granted"){localStorage.setItem(NOTICE_KEY,"1");return;}
  lockWelcome();
