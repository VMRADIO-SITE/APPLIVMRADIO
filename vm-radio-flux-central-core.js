...
const ENGINE='https://admin.vmradio.fr/api/public/radio/nowplaying';
...
const VM_HLS_NATIVE=!!(liveAudio.canPlayType('application/vnd.apple.mpegurl')||liveAudio.canPlayType('application/x-mpegURL'));
...
liveAudio.preload='auto';liveAudio.playsInline=true;liveAudio.setAttribute('playsinline','');liveAudio.setAttribute('webkit-playsinline','');liveAudio.setAttribute('aria-hidden','true');liveAudio.style.cssText='position:absolute;width:1px;height:1px;opacity:0;pointer-events:none;left:-9999px;top:-9999px';
if(liveAudio.getAttribute('src')!==STREAM)liveAudio.src=STREAM;
if(VM_HLS_NATIVE){liveAudio.dataset.vmAudioRecoveryV4='1';liveAudio.dataset.vmAudioControlsV4='1';liveAudio.__vmAudioRecoveryV6=true}
...
function preloadCover(value){
  if(!value)return Promise.resolve(false);
  const v=String(value);
  const cached=coverCache.get(v);
  if(cached)return cached;
  const task=new Promise(resolve=>{
    const probe=new Image();
    probe.onload=()=>resolve(true);
    probe.onerror=()=>{coverCache.delete(v);resolve(false)};
    probe.src=v;
  });
  coverCache.set(v,task);
  return task;
}
function setImage(sel,value,fallback=''){
  const v=String(value||fallback||'').trim();
  if(!v)return;
  preloadCover(v);
  document.querySelectorAll(sel).forEach(el=>{
    if(el.tagName!=='IMG')return;
    el.dataset.vmDesiredCover=v;
    el.onerror=()=>{
      const fb=String(fallback||'').trim();
      if(!fb||el.dataset.vmFallbackApplied==='1')return;
      el.dataset.vmFallbackApplied='1';
      el.dataset.vmDesiredCover=fb;
      el.setAttribute('src',fb);
    };
    if((el.getAttribute('src')||'')===v)return;
    el.dataset.vmFallbackApplied='0';
...
function render(current,next,history){
  if(!current)return;
  /* On lance le téléchargement de la prochaine pochette avant toute mise à jour visuelle. */
  if(next?.cover)preloadCover(next.cover);
  if(current?.cover)preloadCover(current.cover);
  ...
  setImage('[data-current-cover],#currentCover,#cover,.current-cover,.cover-wrap img',current.cover,current?.type==='jingle'?JINGLE_FALLBACK:'');
  if(next){ensureNextTime();const nextAt=nextBroadcastTime(current,next);setText('[data-next-title],#nextTitle,#programNext,.next-title,#nextTrackTitle',next.title);setText('[data-next-artist],#nextArtist,.next-artist,#nextTrackArtist',next.artist);setText('[data-next-time],#nextTime,.next-time',clock(nextAt));setImage('[data-next-cover],#nextCover,.next-cover,.next-card img,#nextTrackCover',next.cover,next?.type==='jingle'?JINGLE_FALLBACK:'')}
